...
(function (Game) {
  ...
  const state = {
    ...
    sent: -1,           // счёт, который сервер принял; -1 — не отправляли ничего
  ...
  // повод пошлёт тот же рекорд заново, и он всё-таки встанет в таблицу.
  //
  // Поводов отправить теперь четыре, а не один. Раньше счёт уходил только на
  // гейм-овере, и это оставляло дыру: игрок, который представился и не доиграл
  // ни одной партии до конца — закрыл вкладку, свернул браузер, ушёл в другое
  // приложение, — в `players` появлялся, а в `scores` не попадал никогда.
  // Отсюда и расхождение между списком игроков и таблицей рейтинга. Поводы:
  //
  //   1. запуск — досылаем рекорд, если сервер его ещё не видел;
  //   2. гейм-овер — рекорд игрока;
  //   3. уход со страницы — счёт незаконченной партии, как есть;
  //   4. возврат на вкладку — досылка того, что не дошло.

  // Одна настоящая отправка. Всё, что решает, отправлять ли вообще, собрано
  // здесь, чтобы поводы выше не повторяли эти проверки каждый по-своему.
  //
  // keepalive нужен поводу 3: браузер рвёт незавершённые запросы при уходе со
  // страницы, и без него отправка пропадала бы ровно тогда, когда она
  // единственная.
  function sendScore(score, playedMs, keepalive) {
    if (!Game.Profile.hasProfile()) return;

    // Ноль очков — законный результат: игрок сыграл и не поймал ничего, его
    // место в таблице такое же настоящее, как чужие сто. А вот партии нулевой
    // длительности не бывает — это значит, что играть ещё не начинали.
    if (!(playedMs > 0) || score < 0) return;

    // Сервер уже принял столько же или больше — слать нечего. Эта проверка не
    // про экономию трафика, а про кулдаун: после каждой **принятой** отправки
    // сервер около 31 секунды отвечает `too_fast`, и потратить его на повтор
    // давно известного числа — значит подставить под отказ следующий
    // настоящий рекорд.
    if (score <= state.sent) return;

    const name = Game.Profile.profile.name;
    const pin = Game.Profile.profile.pin;

    Game.Api.submitScore(name, pin, score, playedMs, keepalive).then(function (res) {
      // Запоминаем только то, что сервер прямо подтвердил. Отказ (`too_fast`,
      // `impossible`) и обрыв сети оставляют state.sent как был, и следующий
      // повод отправит это же число заново — в этом вся самопочинка.
      if (res.ok && res.data === 'ok') {
        state.sent = score;
        Game.Storage.saveSent(score);
      }
    });
  }

  // Поводы 1, 2 и 4: в таблицу идёт рекорд вместе с длительностью той партии,
  // где он поставлен. Пара «счёт + время» обязана быть настоящей — сервер
  // проверяет по ней правдоподобие счёта.
  function syncBest(keepalive) {
    if (!Game.Profile.hasProfile()) return;

    if (state.best > 0 && state.bestMs > 0) {
      sendScore(state.best, state.bestMs, keepalive);
      ...
      sendScore(state.score, state.playedMs, keepalive);
    }
  }

  // Повод 3: игрок уходит со страницы. Если партия при этом идёт — отправляем
  // её счёт как есть, не дожидаясь гейм-овера, которого может и не случиться.
  // Длительность считаем по-настоящему, от старта партии до этой секунды.
  function syncLeaving() {
    if (state.screen === SCREENS.PLAYING && state.startedAt > 0) {
      sendScore(state.score, Math.max(0, Date.now() - state.startedAt), true);
    } else {
      syncBest(true);
    }
  }
